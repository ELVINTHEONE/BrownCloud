...
# Created by RJ Patawaran, http://flask.pocoo.org/snippets/134/
class ChunkedSecureCookieSessionInterface(SessionInterface):
    ...
    def get_signing_serializer(self, app):
        # Note: app should have the following properties: secret_key, session_cookie_name, permanent_session_lifetime
        ...
        if not session:
            if session.modified:
                for cookie_idx in range(10):
                    response.delete_cookie('%s:%s' % (app.session_cookie_name, cookie_idx), domain=domain, path=path)
            return
